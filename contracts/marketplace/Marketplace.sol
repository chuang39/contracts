...
contract Marketplace is Ownable, Pausable, MarketplaceStorage, NativeMetaTransaction {
  ...
  )
  {
    ...
    shouldMint = false;
    setMinesharePerMillion(_minesharePerMillion); // default value = 100000
  }
   ...
   internal returns (Order memory)
  {
    ...
    if (ownerCutPerMillion > 0) {
      ...
      // Transfer share amount for airdropPool address
      require(
        acceptedToken.transferFrom(sender, airdropPool, saleShareAmount),
        "Transfering the cut to the airdrop pool failed"
      );
    }

    if (minesharePerMillion > 0 ) {
        // Calculate sale share
        uint256 mineShareAmount = price.mul(minesharePerMillion).div(1000000);

        if(shouldMint) {
            require(acceptedToken.hasRole(acceptedToken.MINTER_ROLE(), address(this)), "Contract must have minter role to mint Spay");
            require(miningPool != address(0), "Invalid MiningPool Address");
            acceptedToken.mint(miningPool, mineShareAmount);
        } 
    }
  ...
  function setAirdropPoolAddress(address airdropPoolAddress) public onlyOwner {
  ...
  function setShouldMint(bool newshouldMint) public onlyOwner {
    shouldMint = newshouldMint;
  }

  function setMinesharePerMillion(uint256 _minesharePerMillion) public onlyOwner {
